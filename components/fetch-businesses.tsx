...
const FetchBusinesses = () => {
  const { onScrape, stopScrape } = UseScraper();
  const { isPending, setQuery, query, progress } = useBusinesses();
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <>
      <form
        //@ts-expect-error pagesNumber should sent as number
      ...
      <Modal isVisible={isPending} onClose={() => {}} hideCloseButton>
        <Progress text={query} progress={progress} onCancel={stopScrape} />
      </Modal>
    </>
  );
};
